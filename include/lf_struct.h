...
struct TAppFuncDef
{
    ...
    struct TBaseObjectFunc					BaseObject;		// 基本对象
    struct TSmartObjectFunc				    Smarter;		// 智能对象
    struct TPlayObjectFunc					Player;			// 人物
...
{
    struct TNormNpcFunc* Npc;								// _TNormNpc
    struct TPlayObjectFunc* PlayObject;                     // _TPlayObject
    struct TBaseObjectFunc* BaseObject;                     // _TBaseObject
    int nCMDCode;
    char* sRawParam01;
    char* sRawParam02;
    char* sRawParam03;
...
struct TMagicACInfo
{
    ...
    uint8_t btMon;
    uint8_t btHero;
    uint8_t btDefenceHum;
    uint8_t btDefenceMon;
    uint8_t btDefenceHero;
    char* sMagicName;                                       // Assuming string is a char pointer
};

struct TAbility
{
    uint32_t Level;                                         // 0x198  //0x34  0x00
    int32_t AC1;                                            // 0x19A  //0x36  0x02
    int32_t AC2;                                            // 0x19A  //0x36  0x02
    int32_t MAC1;                                           // 0x19C  //0x38  0x04
    int32_t MAC2;                                           // 0x19C  //0x38  0x04
    int32_t DC1;                                            // 0x19E  //0x3A  0x06
    int32_t DC2;                                            // 0x19E  //0x3A  0x06
    int32_t MC1;                                            // 0x1A0  //0x3C  0x08
    int32_t MC2;                                            // 0x1A0  //0x3C  0x08
    int32_t SC1;                                            // 0x1A2  //0x3E  0x0A
    int32_t SC2;                                            // 0x1A2  //0x3E  0x0A
    uint32_t HP;                                            // 0x1A4  //0x40  0x0C
    uint32_t MP;                                            // 0x1A6  //0x42  0x0E
    uint32_t MaxHP;                                         // 0x1A8  //0x44  0x10
    uint32_t MaxMP;                                         // 0x1AA  //0x46  0x12
    uint32_t Exp;                                           // 0x1B0  //0x4C 0x18
    uint32_t MaxExp;                                        // 0x1B4  //0x50 0x1C
    int32_t Weight;                                         // 0x1B8   //0x54 0x20
    int32_t MaxWeight;                                      // 0x1BA   //0x56 0x22  背包
    int32_t WearWeight;                                     // 0x1BC   //0x58 0x24
    int32_t MaxWearWeight;                                  // 0x1BD   //0x59 0x25  负重
    int32_t HandWeight;                                     // 0x1BE   //0x5A 0x26
    int32_t MaxHandWeight;                                  // 0x1BF   //0x5B 0x27  腕力
    int32_t CreditPoint;                                    // 声望
    uint32_t NewValue[30];
...
struct TAbilityNG
{
    uint16_t Level;                                         // 内功等级
    uint16_t NH;                                            // 当前内力值
    uint16_t MaxNH;                                         // 内力值上限
    uint32_t Exp;                                           // 当前内功经验
    uint32_t MaxExp;                                        // 当前内功最高经验
};

struct TAbilityAlcohol
{
    uint16_t Alcohol;                                       // 酒量
    uint16_t MaxAlcohol;                                    // 酒量上限
    uint16_t WineDrinkValue;                                // 醉酒度
    int32_t MedicineLevel;                                  // 药力值等级
    uint16_t MedicineValue;                                 // 当前药力值
    uint16_t MaxMedicineValue;                              // 药力值上限
};
...
struct TUserItemProgress
{
    ...
    uint8_t btNameColor;
    uint8_t btCount;
    uint8_t btShowType;                                     // 进度上的值显示方式(0:不显示; 1:百分比; 2:数值;)
    uint16_t wMax;
    uint16_t wValue;
    uint16_t wLevel;
...
struct TCustomProperty
{
    ...
    uint8_t btBindType;
    uint8_t btShowFlag;
    uint8_t btPercent;                                      // 0: 点数；1: 单件装备%； 2: 全身装备%
    uint8_t btHintModule;
    int32_t nValues[ITEM_PROP_VALUES_COUNT];
};
...
struct TFluteInfo
{
    uint16_t GemIndex;                                      // 宝石索引
    uint16_t GemCount;                                      // 宝石叠加数量
};

struct TUserItem
{
    int32_t MakeIndex;
    uint16_t wIndex;                                        // 物品id
    char Name[ITEM_NAME_LEN + 1];
    uint16_t Dura;                                          // 当前持久值
    uint16_t DuraMax;                                       // 最大持久值
    int32_t btValue[14];

    uint32_t dwHeroM2DressEffect;                           // 特效编号 + boHeroM2DressNoBlend  MakeLong(btHeroM2DressEffect + boHeroM2DressNoBlend)

    uint8_t btUpgradeCount;                                 // 升级次数
    bool boStartTime;                                       // 是否开始计时
    int32_t nLimitTime;                                     // 限时物品 分钟
    uint8_t btHeroM2Light;                                  // HeroM2 SetItemsLight
    uint16_t btNewValue[30];
    ...
    uint16_t wEffect;                                       // 特效编号 新

    // 装备凹槽 chongchong 2015-01-02
    uint8_t btFluteCount;                                   // 凹槽数量             1
    ...
    uint16_t wInsuranceCount;                               // 投保次数
    uint16_t wNewLooks;
    uint16_t wNewShape;

    uint16_t wNewExpand3;
    uint16_t wNewExpand4;

    uint8_t btAddDataByte[USER_ITEM_ADD_DATA_BYTE_COUNT];
    int32_t nAddDataInt[USER_ITEM_ADD_DATA_INT_COUNT];
    char sAddDataText[USER_ITEM_ADD_DATA_TEXT_COUNT][21];   // Assuming string[20] is a 21-byte char array
};

struct TStdItemEffect
{
    int16_t FileIndex;                                      // 物品发光效果 文件编号 0
    uint16_t ImageStart;                                    // 物品发光效果 读取位置
    uint8_t ImageCount;                                     // 物品发光效果 读取张数
    bool IsDrawCenter;                                      // 居中播放
    bool IsDrawNoBlend;                                     // 非透明绘制
    bool IsDrawBelow;                                       // 底层绘制
    int16_t OffsetX;                                        // 物品发光效果 微调X
    int16_t OffsetY;                                        // 物品发光效果 微调Y
    uint16_t Time;                                          // 播放速度
};
...
struct TStdItem
{
    ...
    int32_t NeedLevel;
    int32_t Price;
    uint16_t OverLap;                                       // 是否是重叠物品
    uint8_t Color;                                          // 物品名称颜色
    int32_t Stock;
    int32_t Light;                                          // 数据库增加Light字段 piaoyun 2013-08-01
    ...
    uint16_t Elements[24];

    int32_t InsuranceCurrency;
    int32_t InsuranceGold;

    struct TStdItemEffect BagEffect;                        // 包裹中的物品发光效果
    struct TStdItemEffect BodyEffect;                       // 内观中物品发光效果
    int64_t Effect;                                         // 指针改为64位 2021-01-05
};
...
struct TClientItem
{
    ...
    bool IsBind;                                            // 是否绑定
    uint8_t btFluteCount;

    uint8_t btUpgradeCount;                                 // 升级次数
    uint8_t btHeroM2Light;                                  // HeroM2 SetItemsLight

    int32_t btValue[14];                                    // 附加属性
    uint16_t NewValue[30];
...
struct TMagic
{
    ...
    uint8_t btEffectType;
    uint8_t btEffect;
    uint16_t wSpell;
    uint16_t wPower;
    uint8_t TrainLevel[16];
    int32_t MaxTrain[16];
    uint8_t btTrainLv;                                      // 最高可升级等级
    uint8_t btJob;
    uint16_t wMagicIdx;
    uint32_t dwMagicDelayTime;
    uint16_t wDefSpell;
    uint16_t wDefPower;
    uint16_t wMaxPower;
    uint16_t wDefMaxPower;
    char sDescr[19];
    int32_t CanUpgrade;                                     // 是否允许升级 chongchong 2013-12-06
    int32_t MaxUpgradeLevel;                                // 最高
};
...
{                                       // 84
    ...
    struct TMagic_C Def;
    uint32_t dwInterval;
    uint32_t dwRealInterval;
    uint32_t dwLastUseTick;
};

struct TActorIcon
{
    int16_t nFileIndex;                                     // WIL资源编号
    uint16_t nIconIndex;
    uint8_t nIconCount;
    int16_t nX;
    int16_t nY;
    bool boBlend;
    uint8_t btDrawOrder;                                    // 绘制顺序
    int16_t nPlayTime;                                      // 播放速度
